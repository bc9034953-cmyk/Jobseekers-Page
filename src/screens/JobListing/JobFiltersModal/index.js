...
import {filterTabMenus, datePostedData} from './FilterCategoriesData';
import {useDispatch, useSelector} from 'react-redux';
import {
  setFilterParams,
  setSelectedFilters,
  setTempSelectedFilters,
} from './job-filters-slice';
...
export default function JobFiltersModal() {
  ...
  const selectedFilters = jobFilters?.selectedFilters;
  const tempSelectedFilters = jobFilters?.tempSelectedFilters;
  ...
  const handleApply = () => {
    ...
    tempSelectedFilters?.forEach(item => {
      if (item?.name === 'work_experience') {
        params.push({
          [`${item.name}`]: item.value,
        });
      } else {
        params.push({
          [`JobsSearch[${item.name}]`]: item.value,
        });
      }
    });
  ...
  const renderFilterList = () => {
    switch (activeMenu) {
      ...
      default:
        return (
          <FilterValuesListing data={jobLocations} name="job_location_id" />
        );
    }
  };
